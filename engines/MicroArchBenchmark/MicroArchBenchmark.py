...
class MicroArchBenchmark(vm5k_engine):
    ...
    def workflow(self, comb, hosts, ip_mac):
        ...
        try:
            ...
            cpu_index = [item for sublist in self.cpu_topology
                         for item in sublist]
            cpusets = []
            n_mem = []
            vm_numatune = {}
            ...
            if n_cpu > 1:
                cpusets.append(','.join(str(cpu_index[i])
            ...
            if self.options.mem_share:
                ...
                for i in range(len(comb['dist'])):
                    index = cpu_index[i]
                    for j in range(int(comb['dist'][i])):
                        n_mem.append(str(self.memory_per_cpu /
                                         vm_cpu_mapping[index]))
            ...
            if multi_cpu:
                cmd = '; '.join(['virsh vcpupin vm-multi ' + str(i) + ' '
                                 + str(cpu_index[i]) for i in range(n_cpu)])
            ...
            stress = []
            logger.info(host +
                ': Installing kflops on vms and creating stress action')
            if self.options.cachebench:
                stress.append(self.cpu_kflops([vm for vm in vms]))
            else:
                stress.append(self.cpu_kflops([vm
                    for vm in vms if vm['n_cpu'] == 1]))

            if multi_cpu and not self.options.cachebench:
                logger.info(host +
                    ': Installing numactl and kflops on multicore vms')
                cmd = 'export DEBIAN_MASTER=noninteractive ; ' + \
                    'apt-get update && apt-get install -y  --force-yes numactl'
                inst_numactl = Remote(cmd, [vm['ip']
                    for vm in vms if vm['id'] == 'vm-multi']).run()
                if not inst_numactl.ok:
                    exit()
                self.cpu_kflops([vm for vm in vms if vm['id'] == 'vm-multi'],
                                install_only=True)
                for multi_vm in [vm for vm in vms if vm['id'] == 'vm-multi']:
                    for i in range(multi_vm['n_cpu']):
                        stress.append(Remote('numactl -C ' + str(i) +
                            ' ./kflops/kflops > vm_multi_' + str(cpu_index[i])
                            + '.out ', [multi_vm['ip']]))
            ...
            for p in stress_actions.processes:
                if not p.ok:
                    logger.error(host +
                        ': Unable to start the stress for combination %s',
                        slugify(comb))
                    exit()
            ...
            vms_out = [vm['ip'] + '_' + vm['cpuset']
                       for vm in vms if vm['n_cpu'] == 1]
            comb_dir = self.result_dir + '/' + slugify(comb) + '/'
            if self.options.cachebench:
                get_vms_output = Get(vms_ip, ['{{vms_out}}_rmw.out'],
                ...
                for p in get_vms_output.processes:
                    if not p.ok:
                        logger.error(host +
                            ...
                            slugify(comb))
                        exit()
                get_vms_output = Get(vms_ip, ['{{vms_out}}_memcpy.out'],
                                     local_location=comb_dir).run()
                for p in get_vms_output.processes:
                    if not p.ok:
                        logger.error(host +
                ...
                get_vms_output = Get(vms_ip, ['{{vms_out}}.out'],
                                     local_location=comb_dir).run()
                for p in get_vms_output.processes:
                    if not p.ok:
                        logger.error(host +
            ...
            if multi_cpu:
                for multi_vm in [vm for vm in vms if vm['id'] == 'vm-multi']:
                    get_multi = Get([multi_vm['ip']], ['vm_multi_' +
                        str(cpu_index[i]) + '.out '
                        for i in range(multi_vm['n_cpu'])],
                        local_location=comb_dir).run()
                    for p in get_multi.processes:
                        if not p.ok:
                            logger.error(host +
                                ': Unable to retrieve the vm_multi files ' +
    ...
    def cpu_kflops(self, vms, install_only=False):
        """Put kflops.tgz on the hosts, compile it and optionnaly
        prepare a TaktukRemote"""
