...
class RuBBoS(vm5k_engine_para):
    ...
    def define_parameters(self):
        ...
        parameters = {
            ...
            'nbAppMaxMem': range(1, self.options.nb_app_max_mem + 1),
            'nbDBMaxCore': range(1, self.options.nb_db_max_core + 1),
            'nbDBMaxMem': range(1, self.options.nb_db_max_mem + 1),
            'mapping': [0, 1]}
        ...
        try:
            ...
            for i in range(len(allcpusets)):
                ...
                logger.info(', '.join([vm['id'] + ' ' + vm['ip'] + ' ' +
                    str(vm['n_cpu']) + '(' + vm['cpuset'] + ')' for vm in vms]))
            ...
            vms_client = define_vms(vm_ids_client, ip_mac=available_ip_mac,
