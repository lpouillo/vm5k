...
class vm5k_engine( Engine ):
    ...
    def __init__(self):
        ...
        self.options_parser.add_option("-b", dest = "mig_bw", help = "bandwith used for the migration", type = "int", default = default_mig_speed )
...
#        logger.debug('%s', pprint(self.job_info))
    ...
    def setup_cluster(self):
        ...
        TaktukRemote(' echo "Host *" >> /root/.ssh/config ; echo " StrictHostKeyChecking no" >> /root/.ssh/config; ',
                virsh_setup.hosts, connexion_params = {'user': 'root'}).run()
