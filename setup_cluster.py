...
class Virsh_Deployment(object):
    ...
    def __init__(self, hosts = None, env_name = None, env_file = None, kavlan = None, deployment_file = None, oargrid_job_id = None,):
        ...
        if deployment_file is not None:
            ...
            self.kavlan = kavlan
            self.vms = []
        self.packages_list = ' command-not-found bash-completion nmap qemu-kvm  virtinst libvirt-bin taktuk '
    ...
    def run(self):
        ...
        if self.kavlan is not None:
            self.rename_hosts()
        self.setup_packages('command-not-found bash-completion nmap qemu-kvm  virtinst libvirt-bin')
...
#        self.copy_ssh_keys()
    ...
    def install_packages(self, packages_list = None):
        ...
        if packages_list is None:
            packages_list = self.libvirt_packages
        cmd = 'apt-get update && apt-get install  -y --force-yes '+self.packages_list
